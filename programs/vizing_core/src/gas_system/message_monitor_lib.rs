...
pub mod message_monitor {
    ...
    pub fn slice_message(message: &[u16]) -> Option<([u16; 20], u32, u64, Vec<u16>)> {
        if message.len() < 48 {
            return None;
        }
        let contract_addr = &message[1..21];
        let mut contract_addr_array = [0u16; 20];
    ...
    pub fn slice_transfer(message: &[u16]) -> Option<([u16; 20], u32)> {
        let receiver = &message[1..21];
        let mut receiver_addr_array = [0u16; 20];
